; Adapted for Adria
; 	Changes will be tagged with [XM (and possibly date)
; 	When changes come from another source, I'll _try_ to mention it
;
; Enhanced BASIC for Adria, Version 2.22p5CA
; 04 mar 2021	- Added support for CC65/CA65 support (e.g. .EQU => =)
;		- Added 'A' to version, to indicate Adria support
;		
;
...
;
; [XM] 03 mar 2021
; 	-CC65/CA65 needs other Assembler/Linker directives
	.feature	labels_without_colons	; Tool lazy ;)
	.p816					; Enable 65C816 opcodes
	.smart					; Enable 'smart' register widths
;
...
; [XM] - 04 Mar 2021 	- Split up the ZP usage to support Adria
;			  It should be save to use Supermon816 ZP
;FAC1_e          = expneg+1   ; FAC1 exponent
FAC1_e		= $80		; FAC1 expononent
...
; [XM] 04 mar 2021 - Seems $A8-$AF in ZP are still free
;
...
; [XM] - Should be handled differently
...
;[ XM] - 04 mar 2021	- Ram_base set to $2200 (as we also have some UART
;			  buffers... Might have to relocate these?)
;Ram_base        = $0800     ; start of user RAM (1st 2KB used/reserved)
Ram_base	= $2200

; [XM] 04 Mar 2021 	- Loading at $9400, this should leave us a $400 byte
;			  stack for Supermon816
;Ram_top         = $8000     ; end of user RAM+1 ($8000 is start of ROM)
Ram_top         = $9400     ; end of user RAM+1
...
;
; [XM] 04 Mar 2021
;	- Added routines for Adria and override LOAD and SAVE, as we haven't
;	  implemented this (yet)!
B_CHRIN_NW      =    char_scan
B_CHROUT        =    char_out
M_LOAD          =    a_LOAD
M_SAVE          =    a_SAVE
M_EXIT          =    smon816
...
; The start address can be changed, noting the BIOS/Monitor locations
	...
	.ORG    $9400              ; Start of Basic in ROM
						; C02Monitor starts at $E000
						; C02BIOS starts at $F800

ROM_START
; [XM] 04 Mar 2021	- Let's make sure we are using 8-bit registers
	sep	#$30
;
...
UCOK
; [XM] - For single character handling, we need single quotes
	CMP   #'C'                 ; Check for Cold start
	BEQ   LAB_COLD             ; If yes, go Cold Start
...
; [XM] 04 Mar 2021 - change as indicated by Daryl Rictor
;      TXS                     ; reset stack pointer (X Reg = $FF from above)
	lda	#$01		; Set MSB
	xba
	lda	#$FF		; Set LSB
	tcs			; Set stack to $01FF
; [XM] - Added, just to be sure?
	ldx	#$ff
...
LAB_2D93
;
; [XM] 	- For my own explanation: We are going to do a $55, a $AA and a $00
;	  test (and clear memory on the way ;) )
...
; [XM]	- Somehow we are not getting past $2E/$2F for memory MSB?
...
LAB_133E
; [XM] 04 Mar 2021 - Commented as we are not using LOAD/SAVE
...
; [XM] 08 mar 2021	- Seems this branch always is taken with my ROM, skip...
...
LAB_LOAD
; [XM] - 04 Mar 2021 - Disabled load, just return (for now)
;
...
; [XM] - 04 Mar 2021 - Disabled save, just return (for now)
;     JSR    get_basmem        ; find end of program space
;     LDX    Smeml             ;
...
;
; [XM] 04 Mar 2021	- Changed for Adria
...
LAB_CSMSG
	.byte $0D,$0A
	.byte "65C02 Enhanced BASIC Version 2.22p5CA"
	.byte $0D,$0A
	.byte "with 65C816 support", $0D, $0A
...
; [XM] - Added some routines/entry points/etc for Adria support
	.include "../adria.inc"

_basic_end 	= *
_basic_size 	= _basic_end - Ram_top		; Ram_top == start of basic
.out .concat("BASIC                         $", .sprintf("%04x", Ram_top), "      $", .sprintf("%04x", _basic_end), "    $", .sprintf("%04x", _basic_size), "  (", .sprintf("%05d", _basic_size), ")")
;=======================================================================
